...
# Nothing to do here in the normal case.  fppd loads libfpp-midi.so once at its own
...
if [ ! -f libfpp-midi.so ]; then
    echo "Running fpp-midi PreStart Script - libfpp-midi.so is missing, rebuilding"
    make
fi
